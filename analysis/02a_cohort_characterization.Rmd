...
# Output paths
current_file <- "02a_cohort_characterization"
...
upset_configs <- list(
  ...
  medication_overlap = list(
  title = "Patient Overlap: Medications",
  variables = list(
    "Rituximab"            = list(col = "Med_rituximab",          target = c("yes", "Yes", 1, "1")),
    "Tocilizumab"          = list(col = "Med_tocilizumab",        target = c("yes", "Yes", 1, "1")),
    "MMF"                  = list(col = "Med_mmf",               target = c("yes", "Yes", 1, "1")),
    "MTX"                  = list(col = "Med_mtx",               target = c("yes", "Yes", 1, "1")),
    "JAKi"                 = list(col = "Med_jaki",              target = c("yes", "Yes", 1, "1")),
    "Abatacept"            = list(col = "Med_abatacept",         target = c("yes", "Yes", 1, "1")),
    "Sildenafil"           = list(col = "Med_sildenafil",        target = c("yes", "Yes", 1, "1")),
    "Tadalafil"            = list(col = "Med_tadalafil",         target = c("yes", "Yes", 1, "1")),
    "Bosentan"             = list(col = "Med_bosentan",          target = c("yes", "Yes", 1, "1")),
    "Iloprost"             = list(col = "Med_iloprost",          target = c("yes", "Yes", 1, "1")),
    "Macitentan"           = list(col = "Med_macitentan",        target = c("yes", "Yes", 1, "1")),
    "Ambrisentan"          = list(col = "Med_ambrisentan",       target = c("yes", "Yes", 1, "1")),
    "PPI"                  = list(col = "Med_ppi",               target = c("yes", "Yes", 1, "1")),
    "ACEi"                 = list(col = "Med_acei",              target = c("yes", "Yes", 1, "1")),
    "Nintedanib"           = list(col = "Med_nintedanib",        target = c("yes", "Yes", 1, "1")),
    "ASA"                  = list(col = "Med_asa",               target = c("yes", "Yes", 1, "1")),
    "CCB"                  = list(col = "Med_ccb",               target = c("yes", "Yes", 1, "1")),
    "ARB"                  = list(col = "Med_arb",               target = c("yes", "Yes", 1, "1")),
    "OAC"                  = list(col = "Med_oac",               target = c("yes", "Yes", 1, "1"))
  )
),

classification_overlap = list(
    title = "Patient Overlap: cohort_group vs EUSTAR Classification",
    variables = list(
      "Active"       = list(col = "cohort_group", target = c("Active")),
      "Non_Active"   = list(col = "cohort_group", target = c("Non-Active")),
      "EUSTAR_Active" = list(col = "ACTIVE_AI", target = c("1")),
      "EUSTAR_Non_ACTIVE"     = list(col = "ACTIVE_AI", target = c("0"))
    )
  )
)
```
...
generate_upset_plot <- function(conf, df) {
  ...
  plot_df <- data.frame(Dummy_ID = 1:nrow(df), check.names = FALSE)
  ...
  # 1. Calculate totals for each requested set
  set_totals <- colSums(plot_df[, set_names, drop = FALSE])
  
  # 2. Filter out sets that have exactly 0 patients (Prevents UpSetR crash)
  valid_sets <- set_names[set_totals > 0]
  
  # 3. UpSetR mathematically requires at least 2 populated sets to draw an intersection
  if (length(valid_sets) < 2) {
    return(list(status = "failed", msg = sprintf("Not enough populated categories to compute intersections. Surviving sets: %s", paste(valid_sets, collapse = ", "))))
  }
  
  # Find the largest category dynamically from the surviving sets
  max_group_size <- max(set_totals[valid_sets])
  
  # Generate Plot
  p <- UpSetR::upset(
    plot_df, 
    sets = valid_sets, # CRITICAL: Feed only the non-zero sets to the engine
    keep.order = TRUE,
    order.by = "freq",
  ...
  return(list(status = "success", plot = p, data = plot_df[, c("Dummy_ID", valid_sets), drop = FALSE]))
}
```

```{r upset_execution, fig.width=9, fig.height=9, results='asis'}
