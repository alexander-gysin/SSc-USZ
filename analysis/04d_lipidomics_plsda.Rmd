...
plsda_configs <- list(
  job_active_vs_nonactive = list(
    title = "Active vs. Non-Active (EUSTAR)",
    test_var = "ACTIVE_AI",
    target_groups = c("1"),
    ref_groups = c("0"),
  ...
  job_act_vs_ctrl = list(
    title = "Active vs. Control (EUSTAR)",
    test_var = "ACTIVE_AI",
    target_groups = c("1"),
    ref_groups = c("-1"),
    bin_continuous = FALSE,
...
for (job_id in names(plsda_configs)) {
  ...
  if (res$status == "success") {
    ...
    model_results_list[[job_id]] <- res$model
    ...
    cat(sprintf("### Diagnostic Table {.unlisted .unnumbered}\n\n"))
    
    tbl <- getSummaryDF(res$model) %>% 
      kableExtra::kbl(format = "html", caption = sprintf("Diagnostics: %s", job$title)) %>% 
...
```
`r key_insight("These models are not good.", "R2Y which is the explained variance in the responce variable is too low.", "Negative Q2 means the model is performing worse than pure guessing", "pR2Y is the permutation test ie the probability that randomly shuffled explanatory variables perform at least as good as the model.", "Same for pQ2.", "RMSEE is the root mean square error of estimation.)`
