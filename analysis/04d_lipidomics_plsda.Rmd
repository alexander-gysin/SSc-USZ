...
for (job_id in names(plsda_configs)) {
  ...
  if (res$status == "success") {
    ...
    # Render Plots side-by-side using Patchwork
    cat(sprintf("### Model & VIP: %s {.unlisted .unnumbered}\n\n", job$title))
    
    combined_plot <- res$plots$scores + res$plots$vip + patchwork::plot_layout(widths = c(1, 1.2))
    
    if (interactive() && !isTRUE(getOption('knitr.in.progress'))) { 
      print(combined_plot); cat("\n\n")
    } else { 
      # Print directly to chunk to leverage HTML knitr embedding 
      print(combined_plot)
      cat("\n\n") 
    }
    
    # Render Table
    cat(sprintf("### Top VIP Biomarkers {.unlisted .unnumbered}\n\n"))
