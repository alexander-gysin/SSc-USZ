---
title: "03c_proteomics_clustering"
...
# Configurations
`r what_this_does("Defined configuration parameters for unsupervised molecular clustering.", "Set K-means centers (k) to test different numbers of potential endotypes.", "Selected specific clinical variables to automatically profile against the newly discovered clusters.")`
```{r config}
...
# 1. Molecular Clustering Configs ----------------------------------------------
clustering_configs <- list(
  
  # Test a 2-cluster split (Filtered for Signal)
  k2_split_filtered = list(
    title = "Molecular Endotypes (k=2) | High Variance Only",
    k = 2,
    n_neighbors = 15, 
    include_features = high_var_prots, 
    include_clinical_vars = unname(unlist(clinical_domains)), 
    exclude_clinical_vars = final_excludes # <-- SHIELD ENGAGED!
  ),
  
  # Test a 3-cluster split (Filtered for Signal)
  k3_split_filtered = list(
    title = "Molecular Endotypes (k=3) | High Variance Only",
    k = 3,
    n_neighbors = 15, 
    include_features = high_var_prots, 
    include_clinical_vars = unname(unlist(clinical_domains)), 
