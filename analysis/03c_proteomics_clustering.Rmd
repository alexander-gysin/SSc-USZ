...
# Load Subgroup Scores from 03d (if script has been run) to use as overlays
scores_path <- here::here("output", "03d_proteomics_subgroups", "score_master.rds")
if(file.exists(scores_path)) {
  score_master <- readRDS(scores_path)
  # Left join scores directly to the spine so the clustering engine can access them seamlessly
  master_spine <- master_spine %>% left_join(score_master, by = "Subject_ID")
}
```

# Configurations
`r what_this_does("Defined configuration parameters for unsupervised molecular clustering.", "Calculated a Non-Active specific high-variance protein subset using the global variance engine.", "Set K-means centers (k) to test different numbers of potential endotypes.", "Configured the smart 'overlay_vars' to automatically generate color-coded UMAPs and statistical distribution plots.")`
...
clustering_configs <- list(
  ...
  nonactive_only_k2 = list(
    ...
    include_clinical_vars = unname(unlist(clinical_domains)),
    exclude_clinical_vars = final_excludes,
    # The Smart Routing Variables (Mixing Continuous Scores and Categorical Clinicals)
    overlay_vars = c("PCA_Active_Score", "Sig_Active_Score", "HM_Active_Score", "Age", "Sex")
  )
)
```

# Unsupervised Clustering {.tabset}
`r what_this_does("Calculated UMAP coordinates to map high-dimensional proteomics into 2D space based on biological similarity.", "Applied K-Means clustering to officially assign patients to new molecular groups.", "Cross-referenced the newly discovered groups against the clinical spine to identify defining clinical characteristics.", "Generated nested tabsets to display variables mapped structurally onto the UMAP space alongside their statistical group comparisons.")`
...
for (c_id in names(clustering_configs)) {
  ...
  # Main Tab for this Configuration
  cat(sprintf("\n\n## %s\n\n", conf$title))
  ...
  if (res$status == "success") {
    ...
    # 2. Print Smart Overlays in Nested Tabset
    if (!is.null(res$plots$overlays) && length(res$plots$overlays) > 0) {
      cat("\n\n### UMAP Overlays {.tabset}\n\n")
      
      for (ov_name in names(res$plots$overlays)) {
        # Sub-Tab for each Overlay Variable
        cat(sprintf("\n\n#### %s\n\n", ov_name))
        print(res$plots$overlays[[ov_name]])
        cat("\n\n<br>\n\n")
      }
      
      # Exit the sub-tabset by creating a sibling header for the diagnostic log
      cat("\n\n### Cohort Statistics\n\n")
    }
    
    # 3. Print Diagnostic Histograms (if any variables were dropped due to skew)
    ...
    # 4. Print the Clinical Cross-Reference Table with Log
