...
The computational pipeline currently lacks patient medication histories (e.g., Rituximab, Mycophenolate, Calcium Channel Blockers). If organoids exhibit divergent behavior, consider that they may be responding to circulating pharmaceutical agents in the serum rather than the endogenous vascular disease signature. A clinical audit of autoantibodies and renal function (Creatinine) is provided below to check for extreme uremia or other major toxic confounders.
</div>
...
`r what_this_does("Calculated two independent vascular metrics.", "1. Magnitude Score: The sum of aligned Z-scores (Downregulated proteins are inverted so positive always means pro-disease).", "2. Direction Count Score: Assigns +1 if a protein crosses the biological threshold in the 'disease' direction, and -1 if it crosses in the 'healthy' direction.")`
...
`r why_is_this_done("To ensure the organoid experiment uses undeniable vascular serum, we plotted the Magnitude Score against the Count Score. Patients selected for the experiment MUST sit at the absolute extremes of both approaches.")`
...
if (length(valid_poi) > 1 && dea_res$status == "success") {
  ...
  poi_heatmap <- run_heatmap_engine(
    ...
    title = "Selected Proteins of Interest",
...
`r what_this_does("Exports an extended priority list of extreme patients to CSV, providing the biobank with backup options in case primary aliquots are depleted.")`
