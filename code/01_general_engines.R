...
wrap_ml_engine <- function(X, Y, config, global_config, algorithm, job_name, out_dir, assets_dir, project_colors_func) {
  ...
  for (i in seq_along(outer_folds)) {
    ...
    # Apply SMOTE only to training data (Proteomics)
    if (isTRUE(config$use_smote) && config$omics_type == "proteomics") {
      smote_res <- smotefamily::SMOTE(as.data.frame(X_train), as.numeric(Y_train) - 1)
      X_train <- as.matrix(smote_res$data[, -ncol(smote_res$data)])
      Y_train <- as.factor(ifelse(smote_res$data$class == 1, "Target", "Reference"))
    }

    # Route to Specific Algorithm Worker
  ...
  stability_df <- all_weights %>%
    group_by(Feature) %>%
    summarize(Selection_Frequency = sum(Importance > 0) / global_config$cv$outer_folds,
              Mean_Importance = mean(Importance), .groups = "drop") %>%
    filter(Selection_Frequency >= global_config$feature_stability_threshold) %>%
  ...
  ggsave(file.path(out_dir, paste0(job_name, "_", algorithm, "_ROC.png")), plot = p_roc, width = 6, height = 6, dpi = 300)
  if(!is.null(p_stab)) ggsave(file.path(out_dir, paste0(job_name, "_", algorithm, "_Stability.png")), plot = p_stab, width = 8, height = 6, dpi = 300)

  return(list(metrics = metrics_df, roc_plot = p_roc, stability_plot = p_stab, stability_df = stability_df))
}


# WORKERS 3.1: Machine Learning Prediction -------------------------------

#' Worker: Fit LASSO / Elastic Net
#' Inner CV handles Lambda tuning.
worker_fit_lasso <- function(X_train, Y_train, X_test, params, inner_folds) {
  ...
  cv_fit <- glmnet::cv.glmnet(X_train, y_num, family = "binomial", alpha = params$alpha, weights = wts, nfolds = inner_folds)

  preds <- predict(cv_fit, newx = X_test, s = "lambda.min", type = "response")

  coefs <- as.matrix(coef(cv_fit, s = "lambda.min"))
  weights <- data.frame(Feature = rownames(coefs)[-1], Importance = abs(coefs[-1, 1])) %>% filter(Importance > 0)
...
worker_fit_rf <- function(X_train, Y_train, X_test, params) {
  ...
  # Class weights applied inherently via strata/sampsize in imbalanced RF
  ...
  rf_fit <- randomForest::randomForest(
    ...
    ntree = params$ntree,
    strata = Y_train,
    sampsize = c(min_class_size, min_class_size), # Forces balanced trees
    importance = TRUE
  )
...
worker_fit_gb <- function(X_train, Y_train, X_test, params, inner_folds) {
  ...
  xgb_params <- list(objective = "binary:logistic", max_depth = params$max_depth, eta = params$eta, scale_pos_weight = spw)

  # Inner CV for Early Stopping
  cv_xgb <- xgboost::xgb.cv(params = xgb_params, data = dtrain, nrounds = params$nrounds, nfold = inner_folds, showsd = FALSE, stratified = TRUE, print_every_n = 50, early_stopping_rounds = 10, maximize = FALSE, metrics = "logloss", verbose = 0)
...
worker_fit_plsda <- function(X_train, Y_train, X_test, params) {
  # Using caret to wrap PLS for easier standard prediction outputs
  y_num <- ifelse(Y_train == "Target", 1, 0)
  fit <- caret::plsda(X_train, Y_train, ncomp = params$ncomp)

  preds <- predict(fit, X_test, type = "prob")[, "Target", 1] # Extract Target prob for given ncomp

  # VIP approximation via varImp
...
# WORKERS 3.2: Machine Learning Validation -------------------------------
...
worker_plot_roc <- function(pred_df, alg_name, job_name, project_colors_func) {
  ...
  line_color <- project_colors_func("Active") # Map line to Target color

  ggplot(roc_data, aes(x = FPR, y = TPR)) +
    geom_line(color = line_color, size = 1.2) +
    geom_abline(linetype = "dashed", color = "grey50") +
    theme_minimal() +
    labs(title = sprintf("ROC: %s (%s)", job_name, toupper(alg_name)), subtitle = sprintf("AUROC = %s", auc_val), x = "False Positive Rate", y = "True Positive Rate") +
...
worker_plot_stability <- function(stability_df, alg_name, job_name, project_colors_func) {
  ...
  ggplot(top_features, aes(x = reorder(Feature, Mean_Importance), y = Mean_Importance, fill = Selection_Frequency)) +
    geom_bar(stat = "identity") +
    scale_fill_gradient(low = "grey70", high = bar_color, limits = c(0, 1)) +
    coord_flip() +
    theme_minimal() +
    labs(title = sprintf("Stable Features: %s", toupper(alg_name)), x = "Feature", y = "Mean Importance")
}
